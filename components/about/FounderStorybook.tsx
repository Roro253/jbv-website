...
function useActiveIndex(ref: React.RefObject<HTMLDivElement | null>, count: number) {
...
function Slide({ post, index }: { post: FounderPost; index: number }) {
  ...
  return (
    <li
      className="group snap-center shrink-0 w-[90%] md:w-[70%] xl:w-[60%] 2xl:w-[56%] px-3"
      role="group"
    ...
    >
      ...
      >
        <motion.article
          ...
          className="relative aspect-[16/10] overflow-hidden rounded-3xl ring-1 ring-black/5 shadow-[0_28px_80px_-28px_rgba(0,0,0,0.45)] bg-black/5 will-change-transform transform-gpu"
        >
          ...
          <div className="absolute left-4 top-4 sm:left-6 sm:top-6">
            <div className="inline-flex items-center gap-2 rounded-full bg-black/40 px-3 py-1 text-[11px] sm:text-xs font-medium text-white backdrop-blur">
          ...
          <div className="absolute inset-x-4 bottom-4 sm:inset-x-6 sm:bottom-6">
            <div className="rounded-2xl bg-black/30 backdrop-blur-md p-4 sm:p-5">
              <h3 className="text-white text-2xl sm:text-3xl font-semibold leading-tight">{post.title}</h3>
              <p className="mt-2 text-white/90 text-sm sm:text-base leading-snug line-clamp-2 opacity-0 translate-y-2 transition group-hover:opacity-100 group-hover:translate-y-0 group-focus-within:opacity-100 group-focus-within:translate-y-0">
                {post.summary}
              </p>
            </div>
          </div>
          ...
          <div className="absolute right-4 top-4 sm:right-6 sm:top-6">
            <span className="inline-flex items-center gap-1 rounded-full bg-white/90 text-slate-900 px-3 py-1 text-[11px] sm:text-xs font-medium shadow">
              Read <ArrowUpRight className="h-3.5 w-3.5" />
            </span>
          </div>

          <div className="pointer-events-none absolute inset-0 rounded-3xl [mask:linear-gradient(#000,transparent_60%)] before:absolute before:inset-0 before:rounded-3xl before:bg-[conic-gradient(from_0deg,transparent,white,transparent_30%)] before:opacity-0 group-hover:before:opacity-30 before:transition-opacity before:duration-500" />
        </motion.article>
      </Link>
    </li>
  );
...
export default function FounderStorybook() {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      const targetIndex = (active + 1) % FOUNDER_POSTS.length;
      const w = el.clientWidth;
      el.scrollTo({
        left: targetIndex * w * 0.9,
        behavior: prefersReducedMotion ? "auto" : "smooth",
      });
    }, 8000);
  ...
  return (
    ...
    >
        ...
        <div className="flex items-end justify-between gap-6">
          ...
          <div className="hidden sm:flex items-center gap-2">
            <button
              type="button"
              onClick={() => go(-1)}
              className="inline-flex h-10 w-10 items-center justify-center rounded-full ring-1 ring-black/10 bg-white/70 backdrop-blur hover:bg-white text-slate-800 transition"
              aria-label="Previous"
            >
      ...
      <div
        ref={scrollerRef}
        tabIndex={0}
        className="mt-8 md:mt-10 flex overflow-x-auto snap-x snap-mandatory scrollbar-none outline-none"
        role="region"
